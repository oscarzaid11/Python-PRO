...
@app.route('/submit', methods=['POST'])  # Ruta que recibe los datos del formulario por POST
def submit_form():
    ...
    address = request.form['address']# Obtiene la dirección del formulario
    date = request.form['date']      # Obtiene la fecha del formulario
    # Puedes guardar tus datos o enviarlos por correo electrónico
    return render_template('form_result.html',  # Renderiza la plantilla con los resultados
                           name=name,
                           email=email,
                           address=address,
                           date=date
